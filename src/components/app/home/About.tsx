                ...
                <div className="container mx-auto py-10 grid gap-10">
                    ...
                    <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                        {reports.map((report: any) => (
                            <div key={report.id} className="border p-4 rounded-lg text-center">
                                <h1 className="text-4xl font-bold text-neutral-900 dark:text-neutral-100">{report.count}</h1>
                                <span>{currentLocal === "ru" ? report.name_ru : currentLocal === "en" ? report.name_en : report.name}</span>
                            </div>
                        ))}
                    </div>
                </div>
