...
import { Clock, MessageSquare, Rss } from "lucide-react";
import Link from "next/link";


export default function PublicsFaculty({ news, events, announcements }: { news: any, events: any, announcements: any }) {
    const currentLocale = useLocale();
    const t = useTranslations("FacultyPage.detail");

    return (
        <div className="grid gap-4 my-10">
            {(news.length > 0 || events.length > 0 || announcements.length > 0) ?
                    <React.Fragment>
                        <div className="flex">
