...
export async function generateMetadata({ params, }: Props): Promise<Metadata> {
    ...
    const data = await fetch(`${process.env.BACKEND_URL}/main/${slug}/${topic_slug}/`).then((res) => res.json())
...
async function getTopicData({ slug, topic_slug }: { slug: string, topic_slug: string }) {
    const res = await fetch(`${process.env.BACKEND_URL}/main/${slug}/${topic_slug}/`, { cache: 'force-cache' })
